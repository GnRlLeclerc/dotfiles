...
# Auto switch VSCode theme based on my environment variables $THEME and $THEME_VARIANT
function code() {
    # Get the theme from environment variables
    THEME=$(echo $THEME)
    THEME_VARIANT=$(echo $THEME_VARIANT)

    case "$THEME" in
        "Gruvbox")
            CODE_THEME="Gruvbox Dark Medium" 
            ;;
        "One Dark")
        ...
        "Catppuccin")
            case "$THEME_VARIANT" in
                "Moccha")
                    CODE_THEME="Catppuccin Moccha"
                    ;;
                "Frappe")
                    CODE_THEME="Catppuccin Frappé"
                    ;;
                "Macchiato")
        ...
        "Rose Pine")
            case "$THEME_VARIANT" in
                "Main")
                    CODE_THEME="Rosé Pine"
                    ;;
                "Moon")
                    CODE_THEME="Rosé Pine Moon"
                    ;;
            esac
            ;;
        "Kanagawa")
    ...
    JSON_FILE=~/.config/Code/User/settings.json

    # Use sed to find and replace the line
    sed -i '/"workbench.colorTheme":/c\  "workbench.colorTheme": "'$CODE_THEME'",' "$JSON_FILE"

    # Open code with the rest of the arguments
    /usr/bin/code $@
